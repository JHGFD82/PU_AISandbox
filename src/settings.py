...
def required_models(settings: dict, key: str, *, where: str) -> list[str]:
    ...
    Raises:
        ValueError: If the list is missing, empty, or holds anything other than
                    non-blank strings. Raised while the plugin is being
                    imported, so the plugin loader reports it and skips that
                    plugin rather than letting it run against models nobody
                    chose.
    """

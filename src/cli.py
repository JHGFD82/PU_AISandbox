...
def create_argument_parser(
    ...
    """Build the top-level argument parser that handles every command the tool supports.

    Registers built-in commands (``usage``) and then lets each installed plugin
    add its own subcommands (e.g. ``translate``, ``transcribe``, ``prompt``).
    The resulting parser is what interprets everything typed after
    ``python main.py``.
...
def _dispatch(args: argparse.Namespace, plugins: dict[str, ModePlugin]) -> None:
    ...
    those don't require a professor name. For all other commands, raises
    ``CLIError`` with a helpful message if the professor name or command is
    missing or unrecognised. On success, calls the matching plugin's ``run()``
    method or the built-in ``usage`` handler.

    Args:
        args: The object holding all parsed command-line flags for the current
              run, as returned by ``parser.parse_args()``.
        plugins: A dictionary mapping command names to plugin objects.
    """
...
def main() -> None:
    ...
    typed by the user, and routes the request to the appropriate command
    handler. If anything goes wrong that the user should know about, the error
    message is printed to the terminal and the process exits with a failure
    code (exit code 1) so that scripts can detect the failure.
    """

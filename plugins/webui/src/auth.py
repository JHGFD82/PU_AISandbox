...
class PassphraseBackend:
    ...
    The hash is normally read from the ``WEBUI_PASSPHRASE_HASH`` environment
    variable (set via ``.env``, generated by running
    ``python main.py webui set-passphrase``). If that variable is unset or
    empty, this backend treats every request as already authenticated —
    this is the documented "no gate" mode meant for a strictly
    ``127.0.0.1``-only setup (see docs/webui-plugin-plan.md section 8).
    """
    ...
    def __init__(self, passphrase_hash: str | None = None) -> None:
        ...
        Args:
            passphrase_hash: The bcrypt hash to check submissions against.
                              ``None`` (the normal case) reads it from the
                              ``WEBUI_PASSPHRASE_HASH`` environment variable
                              instead.
        """
        self._hash = (
            passphrase_hash
            if passphrase_hash is not None
            else os.environ.get("WEBUI_PASSPHRASE_HASH", "")
        )
...
def hash_passphrase(passphrase: str) -> str:
    """Hash a plaintext passphrase for storage in WEBUI_PASSPHRASE_HASH.
    ...
    Returns:
        A bcrypt hash string safe to paste into ``.env``.
    """
